...
library PendlePtOracleLib {
    ...
    function getPtToAssetRateRaw(
        ...
        if (expiry <= block.timestamp) {
            ...
            uint256 assetToPtRate = MarketMathCore
                ._getExchangeRateFromImpliedRate(lnImpliedRate, timeToExpiry)
                .Uint();
            return PMath.ONE.divDown(assetToPtRate);
        }
    }

    function getSYandPYIndexCurrent(
        IPMarket market
    ) internal view returns (uint256 syIndex, uint256 pyIndex) {
