...
library PendleLpOracleLib {
    ...
    function getLpToAssetRate(
        ...
        (uint256 syIndex, uint256 pyIndex) = PendlePtOracleLib
            .getSYandPYIndexCurrent(market);
        uint256 lpToAssetRateRaw = _getLpToAssetRateRaw(
            market,
            duration,
            pyIndex
        );
    ...
    function _getLpToAssetRateRaw(
        ...
        if (state.expiry <= block.timestamp) {
            ...
            (int256 rateOracle, int256 rateHypTrade) = _getPtRatesRaw(
                market,
                state,
                duration
            );
            ...
            int256 tradeSize = (cParam.mulDown(comp.totalAsset) - state.totalPt)
                .divDown(PMath.IONE + cParam.divDown(rateHypTrade));
    ...
    function _getPtRatesRaw(
        ...
        rateOracle = PMath.IONE.divDown(
            market.getPtToAssetRateRaw(duration).Int()
        );
